...
def main():
    ...
    parser.add_argument(
        ...
        required=False,
        default="File Converted with PdfItDown",
        type=str,
    )
    parser.add_argument(
        "-d",
    ...
    try:
        if inf is not None:
            if outf is not None and len(inf.split(",")) > 1:
                ...
                cprint(
                    "ERROR! Invalid input provided, check your input and output files",
                    color="red",
                    file=sys.stderr,
                )
                sys.exit(1)
            ...
            cprint(
                "ERROR! You should provide at least one of --inputfile or --directory",
                color="red",
                file=sys.stderr,
            )
            sys.exit(1)
