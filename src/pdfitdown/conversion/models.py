...
class OsPath:
    ...
    def __post_init__(self) -> None:
        if self.type == "file" and not Path(self.path).is_file():
        ...
        elif self.type == "directory" and Path(self.path).is_dir() and len(os.listdir(self.path)) == 0:
            raise ValueError(f"Directory {self.path} exists but is empty. Provide a non-empty directory")
            ...
            if self.overwrite:
                warnings.warn(f"File {self.path} already exists and will be overwritten", FileExistsWarning)
            else:
                raise FileExistsError(f"File {self.path} already exists. If you wish to overwrite, please set `overwrite` to True")
        else:
            return
...
class MultipleConversion:
    ...
    def __post_init__(self) -> None:
        if len(self.input_files) != len(self.output_files):
            raise ValueError("There should be as many output files as there are input files")