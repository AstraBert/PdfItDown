...
def all_files() -> list[str]:
    initial_files = ["test.txt", "test0.png", "test1.pptx", "test2.md", "test3.json", "test4.docx", "test5.zip", "test6.xml", "test7.xslx"]
...
def test_ospath_notype(to_convert_dir: str, text_file: str) -> None:
    ...
    path = OsPath(path=text_file.replace(".txt", ".pdf"), type="outputfile", overwrite=True)
    assert path.file_type == "none"
