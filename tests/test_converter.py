...
def test_converter_text_file(text_file: str, converter: Converter) -> None:
    converter.convert(file_path=text_file, output_path=text_file.replace(Path(text_file).suffix, ".pdf"), title="hello world")
...
def test_converter_pdf_file(pdf_file: str, converter: Converter) -> None:
    converter.convert(file_path=pdf_file, output_path=str(pdf_file.replace(Path(pdf_file).suffix, ".1.pdf")), title="hello world")
...
def test_converter_nontext_file(to_convert_file: str, converter: Converter) -> None:
    converter.convert(file_path=to_convert_file, output_path=str(to_convert_file.replace(Path(to_convert_file).suffix, ".pdf")), title="hello world")
...
def test_converter_image_file(image_file: str, converter: Converter) -> None:
    converter.convert(file_path=image_file, output_path=str(image_file.replace(Path(image_file).suffix, ".pdf")), title="hello world")
...
def test_converter_singlefile_erorr(text_file: str, converter: Converter) -> None:
    ...
    with pytest.raises(FileExistsError):
        converter.convert(file_path=text_file, output_path=text_file.replace(Path(text_file).suffix, ".pdf"), title="hello world", overwrite=False)
    with pytest.raises(FileNotFoundError):
        converter.convert(file_path=text_file+"t", output_path=str(text_file.replace(Path(text_file).suffix, ".1.pdf")), title="hello world")
    os.remove(text_file.replace(Path(text_file).suffix, ".pdf"))

def test_converter_multiple_files_default(all_files: list[str], converter: Converter) -> None:
...
def test_converter_multiple_files_custom(all_files: list[str], converter: Converter) -> None:
...
def test_convert_multiple_files_error(all_files: list[str], converter: Converter) -> None:
...
def test_convert_directory(to_convert_dir: str, converter: Converter, all_files: list[str]) -> None:
